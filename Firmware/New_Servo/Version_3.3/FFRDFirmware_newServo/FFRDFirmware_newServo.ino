...
void loop()
{
  while (Serial.available() > 0)
  {
    ...
    if ((Serial.read() == '\n') || (Serial.read() == -1))
    {
      ...
      if (CMD_INDEX(cmd) == Q_TILT)
      {
        ...
        if (degree != arm_pos[arm_index])
        {
          if (degree < arm_pos[arm_index])
          {
            ...
            {
              arm.WritePos(arm_index + 1, degree + 65, 200); //ID:arm_index+1  Pos:pos  velocity:100
              arm.WritePos(arm_index + 1, degree + 60, 50);
            }
            else if ((arm_pos[arm_index] - degree > 30) && (arm_pos[arm_index] - degree <= 60))
            {
              arm.WritePos(arm_index + 1, degree + 65, 180);
              arm.WritePos(arm_index + 1, degree + 60, 50);
            }
            else if ((arm_pos[arm_index] - degree > 10) && (arm_pos[arm_index] - degree <= 30))
            {
              arm.WritePos(arm_index + 1, degree + 65, 160);
              arm.WritePos(arm_index + 1, degree + 60, 50);
            }
            else
            {
              arm.WritePos(arm_index + 1, degree + 62, 150);
              arm.WritePos(arm_index + 1, degree + 60, 50);
            }
          }
          else
          {
            ...
            {
              arm.WritePos(arm_index + 1, degree + 55, 200); //ID:arm_index+1  Pos:pos  velocity:100
              arm.WritePos(arm_index + 1, degree + 60, 50);
            }
            else if ((degree - arm_pos[arm_index] > 30) && (degree - arm_pos[arm_index] <= 60))
            {
              arm.WritePos(arm_index + 1, degree + 55, 180);
              arm.WritePos(arm_index + 1, degree + 60, 50);
            }
            else if ((degree - arm_pos[arm_index] > 10) && (degree - arm_pos[arm_index] <= 30))
            {
              arm.WritePos(arm_index + 1, degree + 55, 160);
              arm.WritePos(arm_index + 1, degree + 60, 50);
            }
            else
            {
              arm.WritePos(arm_index + 1, degree + 58, 150);
              arm.WritePos(arm_index + 1, degree + 60, 50);
            }
          }
      ...
      {
        Serial.println("DaVinci Firmware <version 3.3>");
        Serial.flush();
      }
