...
#define Q_USB_PLUG_IN          0x62
#define Q_USB_PLUG_OUT         0x63
...
#define RELAY_USB_VCC_PIN         0
#define RELAY_USB_DATA_MINUS_PIN  1
#define RELAY_USB_DATA_PLUS_PIN   2    //PIN 12 will influence reading speed and angle
#define RELAY_USB_GND_PIN         3
...
void setup() 
{
  ...
  pinMode(RELAY_USB_VCC_PIN, OUTPUT);
  pinMode(RELAY_USB_DATA_MINUS_PIN, OUTPUT);
  pinMode(RELAY_USB_DATA_PLUS_PIN, OUTPUT);
  pinMode(RELAY_USB_GND_PIN, OUTPUT);  
  digitalWrite(RELAY_USB_VCC_PIN, LOW);
  digitalWrite(RELAY_USB_DATA_MINUS_PIN, LOW);
  digitalWrite(RELAY_USB_DATA_PLUS_PIN, LOW);
  digitalWrite(RELAY_USB_GND_PIN, LOW);
}
...
void loop() 
{
  while (Serial.available() > 0) 
  {
    ...
    if ((Serial.read() == '\n') || (Serial.read() == -1))
    {
      ...
      {
        Serial.println("DaVinci Firmware <version 3.1>");
        Serial.flush();
      }
      ...
      else if (CMD_INDEX(cmd) == Q_USB_PLUG_IN)
      {
        Serial.println("Turn off Relay to plug in USB device!");
        Serial.flush();
        digitalWrite(RELAY_USB_VCC_PIN, LOW);            // turn No.1 Relay off (VCC) 
        digitalWrite(RELAY_USB_GND_PIN, LOW);            // turn No.4 Relay off (GND)
        delay(500);
        digitalWrite(RELAY_USB_DATA_MINUS_PIN, LOW);     // turn No.2 Relay off (USB Data -)
        digitalWrite(RELAY_USB_DATA_PLUS_PIN, LOW);      // turn No.3 Relay off (USB Data +)  
      }
      else if (CMD_INDEX(cmd) == Q_USB_PLUG_OUT)
      {
        Serial.println("Turn on Relay to plug out USB device!");
        Serial.flush(); 
        digitalWrite(RELAY_USB_DATA_MINUS_PIN, HIGH);    // turn No.2 Relay on (USB Data -)
        digitalWrite(RELAY_USB_DATA_PLUS_PIN, HIGH);     // turn No.3 Relay on (USB Data +) 
        delay(500);
        digitalWrite(RELAY_USB_VCC_PIN, HIGH);           // turn No.1 Relay on (VCC) 
        digitalWrite(RELAY_USB_GND_PIN, HIGH);           // turn No.4 Relay on (GND)
      }
