...
#define Q_POWER_BUTTON_PUSHER_PRESS_ANGLE    80
#define Q_POWER_BUTTON_PUSHER_RELEASE_ANGLE  100
...
#define LIFTER            0       //0 for not having this hardware, 1 for having this hardware. Default value is 1 for full version, 0 for accessory.
...
#define USB_CUTTER        0       //0 for not having this hardware, 1 for having this hardware. Default value is 1 for full version, 1 for USB cutter accessory, 0 for other accessory.
  ...
  while (Serial.available() > 0)
  {
    ...
    if ((Serial.read() == '\n') || (Serial.read() == -1))
    {
      ...
      else if (CMD_INDEX(cmd) == Q_Z_ROTATE)
      {
        ...
        int rotate_direction = ((cmd & 0x00F00000) >> 20);		//0 for clockwise, 1 for anti-clockwise
        ...
        double degree_servo = degree * 2.414 * 1.5;  //70 * 1.5 : 29 = 105 £º 29
        ...
        double speed_servo = speed_z * 2.414 * 1.5;  //70 * 1.5 : 29 = 105 £º 29
      ...
      {
        Serial.println("DaVinci Firmware <version 5.1>");
        Serial.flush();
      }
