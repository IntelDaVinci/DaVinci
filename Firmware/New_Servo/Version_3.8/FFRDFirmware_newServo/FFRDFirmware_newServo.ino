...
#define Q_ONE_QUICK_USB_START  0x53
#define Q_ONE_QUICK_USB_STOP   0x54
#define Q_TWO_QUICK_USB_START  0x55
#define Q_TWO_QUICK_USB_STOP   0x56
#define Q_TILT                 0x70
...
void loop()
{
  while (Serial.available() > 0)
  {
    ...
    if ((Serial.read() == '\n') || (Serial.read() == -1))
    {
      ...
      {
        Serial.println("DaVinci Firmware <version 3.8>");
        Serial.flush();
      }
      ...
      else if (CMD_INDEX(cmd) == Q_ONE_QUICK_USB_START)
      {
        Serial.println("Start quick on-off of No.1 USB!");
        Serial.flush();
        while (1)
        {
          digitalWrite(RELAY_ONE_PIN, HIGH);  // turn No.1 Relay on
      ...
      else if (CMD_INDEX(cmd) == Q_ONE_QUICK_USB_STOP)
      {
        Serial.println("Stop quick on-off of No.1 USB!");
        Serial.flush();
        digitalWrite(RELAY_ONE_PIN, LOW);   // turn No.1 Relay off
      }
      else if (CMD_INDEX(cmd) == Q_TWO_QUICK_USB_START)
      {
        Serial.println("Start quick on-off of No.2 USB!");
        Serial.flush();
        while (1)
        {
          digitalWrite(RELAY_TWO_PIN, HIGH);  // turn No.2 Relay on
      ...
      else if (CMD_INDEX(cmd) == Q_TWO_QUICK_USB_STOP)
      {
        Serial.println("Stop quick on-off of No.2 USB!");
        Serial.flush();
        digitalWrite(RELAY_TWO_PIN, LOW);   // turn No.2 Relay off
      }
