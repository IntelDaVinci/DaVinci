...
namespace DaVinci
{
    KeywordCheckHandlerForNetworkIssue::KeywordCheckHandlerForNetworkIssue(boost::shared_ptr<KeywordRecognizer> keywordRecog)
    {
        ...
        this->needSemantics = true;
        this->needNegative = true;
        this->maxLineInterval = 60;

        std::map<std::string, std::vector<Keyword>> keywordList;
        string keywordCategory = KeywordUtil::Instance().KeywordCategoryToString(KeywordCategory::NETWORKISSUE);
    ...
    void KeywordCheckHandlerForNetworkIssue::InitializeKeywordList(std::map<std::string, std::vector<Keyword>> &keywordList)
    {
    ...
    bool KeywordCheckHandlerForNetworkIssue::HandleRequestImpl(std::string &language, std::vector<Keyword> &enTexts, std::vector<Keyword> &zhTexts)
    {
        ...
        // Did't find keyword in blacklist, return HandleCommonRequestImpl()
        return HandleCommonRequestImpl(language, enTexts, zhTexts);
    }

    FailureType KeywordCheckHandlerForNetworkIssue::GetCurrentFailureType()
    {
        return FailureType::WarMsgNetwork;
    }

    cv::Rect KeywordCheckHandlerForNetworkIssue::GetOkRect(const cv::Mat &frame)
    {
        return EmptyRect;
    }
}