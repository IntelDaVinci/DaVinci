...
#ifndef _SHARINGKEYWORDRECOGNIZER_
#define _SHARINGKEYWORDRECOGNIZER_

#include "KeywordRecognizer.hpp"

namespace DaVinci
{
    class SharingKeywordRecognizer
    {
    public:
        SharingKeywordRecognizer();

        void InitializeKeywordList(std::map<std::string, std::vector<Keyword>> &keywordList);

        bool RecognizeKeyword(std::vector<Keyword> allKeywords, const std::string &languageMode);
