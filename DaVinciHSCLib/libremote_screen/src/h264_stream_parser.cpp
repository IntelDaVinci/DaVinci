...
bool CH264VideoParser::u(mfxU8 n)
{
...
void CH264VideoParser::parse()
{
    m_nOffsetInStream = 0;

    while (m_nOffsetInStream < (m_nDataLength << 3))
    {
        mfxU64 curByteInStream = (m_nOffsetInStream >> 3);
...
//                    m_uData[0], m_uData[1], m_uData[2], m_uData[3], m_uData[4], m_uData[5], m_uData[6], m_uData[7], m_uData[8], m_uData[9], m_uData[10], m_uData[11], m_uData[12], m_uData[13], m_uData[14], m_uData[15], m_uData[16], m_uData[17], m_uData[18], m_uData[19], m_uData[20]);
        ...
        case SCAN_SPS_FOR_NUM_REF_FRAMES_IN_PIC_ORDER_CNT_TYPE:
            if (uev())
            {
                ...
                {
                    h264info.sps.offset_for_ref_frame = new mfxI64[(mfxU32)(h264info.sps.num_ref_frames_in_pic_order_cnt_cycle)];
                    m_nArraryIndex = 0;
        ...
        case SCAN_SPS_FOR_PIC_HEIGHT_IN_MAP_UNITS_MINUS1:
            if (uev())
            {
                ...
                m_oObserver->OnFrameSize(this, (h264info.sps.pic_width_in_mbs_minus1 + 1) * 16, (h264info.sps.pic_height_in_map_units_minus1 + 1) * 16);
        ...
        case SCAN_SLICE_HEADER_FOR_FRAME_NUM:
            if (u((mfxU8)(h264info.sps.log2_max_frame_num_minus4) + 4))
            {
                mfxU64 frame_num = value();
                if (frame_num != h264info.frame_num)
                {
                    h264info.frame_num = frame_num;
                    m_oObserver->OnNewFrame(this, h264info.frame_num);
                }
                state = SCAN_SLICE_HEADER_FOR_FIELD_PIC_FLAG;
            }
            break;
        case SCAN_SLICE_HEADER_FOR_FIELD_PIC_FLAG:
            if (!h264info.sps.frame_mbs_only_flag)
            {
                if (u(1))
                {
                    h264info.field_pic_flag = value() ? true : false;
                    if (h264info.field_pic_flag)
                    {
                        state = SCAN_SLICE_HEADER_FOR_BOTTOM_FIELD_FLAG;
                    }
                    else
                    {
                        state = SCAN_SLICE_HEADER_FOR_IDR_PIC_ID;
                    }
                }
            }
            else
            ...
            break;
        case SCAN_SLICE_HEADER_FOR_BOTTOM_FIELD_FLAG:
            if (u(1))
            {
                h264info.bottom_field_flag = value() ? true : false;
                state = SCAN_SLICE_HEADER_FOR_IDR_PIC_ID;
            }
            break;
        case SCAN_SLICE_HEADER_FOR_IDR_PIC_ID:
            if (h264info.idr_pic_id)
            {
                if (uev())
                {
                    h264info.idr_pic_id = value();
                    state = SCAN_SLICE_HEADER_FOR_PIC_ORDER_CNT_LSB;
                }
            }
            else
            {
            ...
            break;
        case SCAN_SLICE_HEADER_FOR_PIC_ORDER_CNT_LSB:
            if (h264info.sps.pic_order_cnt_type == 0)
            {
                if (u(h264info.sps.log2_max_pic_order_cnt_lsb_minus4 + 4))
                {
                    h264info.pic_order_cnt_lsb = value();
                    state = SCAN_NAL_ALIGN_BYTE;
                }
            }
            else
...
//            LOGI("nal_unit_type:%u, \tlog2_max_frame_num_minus4:%llu, \tpic_order_cnt_type:%llu, \tpic_order_cnt_lsb:%llu, \tframe_num:%llu, \tslice_type:%llu, \t(%llux%llu)\n", h264info.nal_unit_type, h264info.sps.log2_max_frame_num_minus4, h264info.sps.pic_order_cnt_type, h264info.pic_order_cnt_lsb, h264info.frame_num, h264info.slice_type, (h264info.sps.pic_width_in_mbs_minus1 + 1) * 16, (h264info.sps.pic_height_in_map_units_minus1 + 1) * 16);
